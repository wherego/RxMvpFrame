package com.yangzheandroid.rxmvp.widget.refreshload;
...
public class PullRefreshLayout extends RelativeLayout {
	...
	private OnRefreshLoadListener mOnRefreshListener; // 刷新回调接口
	...
	private void initView(Context context) {
		...
		refreshView = LayoutInflater.from(context).inflate(R.layout.base_refresh, this, false);
		loadMoreView = LayoutInflater.from(context).inflate(R.layout.base_load, this, false);
		addView(refreshView);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		mPullableView.layout(0, (int) (pullDownY + pullUpY), mPullableView.getMeasuredWidth(),
				(int) (pullDownY + pullUpY) + mPullableView.getMeasuredHeight());
		loadMoreView.layout(0, (int) (pullDownY + pullUpY) + mPullableView.getMeasuredHeight(),
				loadMoreView.getMeasuredWidth(),
				(int) (pullDownY + pullUpY) + mPullableView.getMeasuredHeight() + loadMoreView.getMeasuredHeight());
	}
	...
	public void refreshFinish(boolean isSuccess) {
		...
		if (showRefreshResultEnable) {
			if (isSuccess) { // 刷新成功
				...
				refreshHintText.setCompoundDrawablePadding(DeviceUtil.dp2px(getContext(), 10));
			} else { // 刷新失败
	...
	public void loadFinish(boolean isSuccess) {
		...
		if (showRefreshResultEnable) {
			if (isSuccess) { // 加载更多成功
				...
				loadHintText.setCompoundDrawablePadding(DeviceUtil.dp2px(getContext(), 10));
			} else { // 加载更多失败
	...
	private class AutoRefreshAndLoadTask extends AsyncTask<Integer, Float, String> {
		...
		protected void onPostExecute(String result) {
			...
			// 刷新操作
			if (mOnRefreshListener != null)
				mOnRefreshListener.onRefresh(PullRefreshLayout.this);
			hide();
		}
