    ...
    synchronized void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        switch (requestCode) {
            case REQUEST_CODE_PERMISSION:
                ...
                LinkedList<String> deniedPermissions_state = new LinkedList<>();
                ...
                if (!grantedPermissions.isEmpty() && deniedPermissions.isEmpty()) {
                    ...
                    if (!deniedPermissions.isEmpty()) {
                        for (String per : deniedPermissions) {
                            if (mService.shouldShowRequestPermissionRationale(mActivity, per)) {
                                deniedPermissions_state.add(per);
                            }
                        }

                        Log.e(TAG, "deniedPermissions_state.size()==   " + deniedPermissions_state.size());
                        //只有在 拒绝提示后才弹出此类对话框
                        if (deniedPermissions_state.size() == 0) {
                        ...
                        Toast.makeText(mActivity, "deniedPermissions.isEmpty()", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
        }
