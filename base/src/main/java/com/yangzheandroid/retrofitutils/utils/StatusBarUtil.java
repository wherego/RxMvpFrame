...
import com.yangzheandroid.retrofitutils.widget.StatusBarView;
...
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, int color, int statusBarAlpha) {
        ...
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {//如果版本大于4.4
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);//设置状态栏透明
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            int count = decorView.getChildCount();
            //设置状态栏的颜色，如果最后一个是Status，则直接设置，如果不是自己画一个同样高度的状态栏
    ...
    /**
     * 为 fragment 头部是 ImageView 的设置状态栏自定义透明
     *
     * @param activity       fragment 对应的 activity
     * @param statusBarAlpha 状态栏透明度
     * @param needOffsetView 需要向下偏移的 View
